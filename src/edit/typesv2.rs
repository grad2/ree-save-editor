...
use crate::{
    edit::{EditContext, EditResponse, Editable, copy::CopyBuffer},
    save::{remap::Remap, types::*},
};
...
fn edit_enum_from_field(value: &mut i64, ui: &mut Ui, ctx: &mut EditContext) -> EditResponse {
    ...
    let enum_text = ctx.field_info.and_then(|field_info| {
        let x = enum_str.and_then(|enum_str| {
            ctx.type_map
                .get_enum_text(enum_str, &field_info.original_type, ctx.language)
        });
        x
...
fn edit_i32_enum_from_type(
    ...
    let enum_str = enum_type.and_then(|e| e.get(&value.to_string()));
    let enum_text = enum_str.and_then(|enum_str| {
        ctx.type_map
            .get_enum_text(enum_str, enum_type_str, ctx.language)
    });
    ui.horizontal(|ui| {
        match enum_str {
            Some(mut enum_str) => {
                ...
                ComboBox::from_id_salt(ctx.id)
                    ...
                    .show_ui(ui, |ui| {
                        for option in enum_list {
                            let enum_text =
                                ctx.type_map
                                    .get_enum_text(option, enum_type_str, ctx.language);
                            let preview = if let Some(ref enum_text) = enum_text {
                                format!("{option}:\"{enum_text}\"")
                            } else {
                                option.to_string()
            ...
            Some(mut enum_str) => {
                ...
                ComboBox::from_id_salt(ctx.id)
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        if let Some(x) = enum_type.get(enum_str).and_then(|e| e.parse::<u32>().ok())
                        {
                            *value = x;
                        }
                    });
...
pub fn get_enum_preview<T: Display>(
    v: T,
    enum_type_str: &str,
    ctx: &mut EditContext,
) -> Option<String> {
...
impl FieldValue {
    pub fn get_preview(&self, ctx: &mut EditContext) -> Option<String> {
        match self {
            ...
            FieldValue::Array(a) => a.values[0].get_preview(ctx).map(|s| format!("[0]={}", s)),
    ...
    fn edit(&mut self, ui: &mut Ui, ctx: &mut EditContext) -> EditResponse {
        ui.push_id(ctx.id, |ui| {
            let is_search_active = (!ctx.search_paths.is_empty()
                || !ctx.search_leaf_nodes.is_empty())
                && !ctx.search_found_leaf;
            let is_path = ctx
                .field_info
                .map(|f| ctx.search_paths.contains(&(f.type_hash, f.hash)))
                .unwrap_or(false);
            ...
            ui.scope(|ui| {
                ...
                let mut row_heights =
                    ui.data_mut(|d| d.get_temp::<Vec<f32>>(state_id).unwrap_or_default());

                if row_heights.len() != self.values.len() {
                    row_heights.resize(self.values.len(), target_height);
                }
                let (visible_sum, visible_count): (_, u32) = row_heights
                    .iter()
                    .enumerate()
                ...
                let total_content_height =
                    visible_sum + (visible_count.saturating_sub(1) as f32 * spacing);

                let max_height = if ctx.depth == 0 {
                    let h = ui.available_height();
                    if h.is_infinite() { 1000.0 } else { h - 10.0 }
                } else {
                    700.0
                };
                        ...
                        ScrollArea::vertical()
                            ...
                            .show(ui, |ui| {
                                ...
                                for (i, member) in self.values.iter_mut().enumerate() {
                                    if !ctx.search_range.contains(&i) {
                                        continue;
                                    }

                                    let cached_h = row_heights[i];
                                    let add_spacing =
                                        if i < last_visible_index { spacing } else { 0.0 };
                                    ...
                                    if current_y > clip_rect.max.y + 200.0 {
                                        let (rem_h, rem_c): (_, u32) = row_heights[i..]
                                            .iter()
                                            .enumerate()
                                            .filter(|(offset, _)| {
                                                ctx.search_range.contains(&(i + offset))
                                            })
                                            .fold((0.0, 0), |(acc_h, acc_c), (_, h)| {
                                                (acc_h + h, acc_c + 1)
                                            });
                                    ...
                                    if default_open.is_some() || !is_search_active {
                                        let preview =
                                            member.get_preview(ctx).unwrap_or("".to_string());
                                        ...
                                        match array_type {
                                            ArrayType::Class => {
                                                let id_salt = if expand {
                                                    (i, "search_mode", ctx.id)
                                                } else {
                                                    (i, "normal", ctx.id)
                                                };
                                                let header_id = ui.make_persistent_id(id_salt);
                                                let class_label = member
                                                    .as_class()
                                                    .and_then(|c| ctx.type_map.get_by_hash(c.hash))
                                                    .map(|c| c.name.clone());
                                                let label = if let Some(class_label) = class_label {
                                                    //format!("{i}: {preview}:{class_label}")
                                                    format!("{i}: {preview}")
                                                } else {
                                                    format!("{i}: {preview}")
                                                };
                                                collapsing_header_with_buttons_for_array_member(
                                                    ui,
                                                    header_id,
                                                    default_open,
                                                    label,
...
impl Editable for Field {
    fn edit(&mut self, ui: &mut Ui, ctx: &mut EditContext) -> EditResponse {
        ...
        let name: String = field_info.map(|x| x.name.clone()).unwrap_or_else(|| {
        ...
        let og_type = field_info
            .map(|f| f.original_type.clone())
            .unwrap_or_else(|| format!("{:?}", self.field_type));
        ...
        let is_leaf = field_info
            .map(|field_info| {
                ctx.search_leaf_nodes
                    .contains(&(field_info.type_hash, self.hash))
            })
            .unwrap_or(false);
        let is_path = field_info
            .map(|field_info| {
                ctx.search_paths
                    .contains(&(field_info.type_hash, self.hash))
            })
        ...
        let id_salt = if has_search && !ctx.search_found_leaf {
            (self.hash, "search_mode", ctx.id)
        } else {
            (self.hash, "normal", ctx.id)
        };
        let default_open = if expand { Some(true) } else { None };
        ...
        match &self.field_type {
            FieldType::Array | FieldType::Class => {
                let header_id = ui.make_persistent_id(id_salt);
                collapsing_header_with_buttons_for_field(
                    ui,
                    header_id,
                    default_open,
                    name_contained,
            ...
            FieldType::Struct => match og_type.as_str() {
                "via.rds.Mandrake" => {
                    let header = CollapsingHeader::new(name_contained).id_salt(id_salt);
                    let header = if let Some(state) = default_open {
                        header.default_open(state)
                    } else {
                        header
                    };
                    header.show(ui, |ui| {
...
impl Class {
    fn edit_as_bitset(
            ...
            for v in &mut a.values {
                ...
                for i in 0..32 {
                        ...
                        if let Some(series) = series_enum.get(&series.to_string()) {
                            if let Some(part_enum) = part_enum {
                                if let Some(part) = part_enum.get(&part.to_string()) {
                                    specific = ctx.type_map.get_enum_text(
                                        &format!("{series}{part}"),
                                        "app.ArmorDef.SpecificPiece",
                                        ctx.language,
                                    );
                                }
                            }
                    ...
                    if let Some(specific) = specific {
                        ...
                        ui.checkbox(
                            &mut bit_val,
                            format!("idx={idx}:({gender}:\"{series}\":{part})"),
                        );
                        x.set_bit(i as usize, bit_val);
                    }
                }
        ...
        CollapsingHeader::new("_Value").show(ui, |ui| {
            eframe::egui::Frame::new()
                .fill(ui.visuals().faint_bg_color)
                .stroke(ui.visuals().widgets.noninteractive.bg_stroke)
                .inner_margin(4.0)
                .show(ui, |ui| {
                    ScrollArea::vertical()
                        ...
                        .show(ui, |ui| match generic {
                            Some("Custom.OuterArmorFlags") => {
                                edit_outer_armor_flags(ui, &mut values)
                            }
    ...
    fn add_enum_edit(
        ...
        ui: &mut Ui,
        ctx: &mut EditContext,
        left: &mut HashSet<u32>,
    ) -> EditResponse {
        self.add_enum_edit_ex(
            type_info,
            field_name,
            field_name,
            enum_type_str,
            ...
            match field.value {
                FieldValue::S32(mut val) => {
                    ui.horizontal(|ui| {
                        ...
                        edit_i32_enum_from_type(&mut val, enum_type_str, ui, &mut ctx);
                ...
                FieldValue::U16(val) => {
                    ui.horizontal(|ui| {
                        ...
                        let child_id = ui.make_persistent_id(hash).value();
                        let mut ctx = Class::udpate_ctx(type_info, child_id, ctx);
                        let mut val_u32 = val as u32;
                        edit_u32_enum_from_type(&mut val_u32, enum_type_str, ui, &mut ctx);
                        field.value = FieldValue::U16(val_u32 as u16);
                    });
                    left.remove(hash);
                }
                FieldValue::S16(val) => {
                    ui.horizontal(|ui| {
                        ...
                        edit_i32_enum_from_type(&mut val_i32, enum_type_str, ui, &mut ctx);
                ...
                FieldValue::U32(val) => {
                    ui.horizontal(|ui| {
                        ...
                        let mut ctx = Class::udpate_ctx(type_info, child_id, ctx);
                        let mut val = val;
                        edit_u32_enum_from_type(&mut val, enum_type_str, ui, &mut ctx);
                        field.value = FieldValue::U32(val as u32);
                    });
                    left.remove(hash);
                }
                _ => {}
    ...
    fn get_enum_preview_from_field(
        &self,
        field_name: &str,
        enum_type_str: &str,
        ctx: &mut EditContext,
        ...
        match field_value {
            FieldValue::S32(val) => {
                return ctx.type_map.get_enum_str(val, enum_type_str).map(
                    |s| match get_enum_preview(val, enum_type_str, ctx) {
                        Some(text) => text,
                        None => s.clone(),
                    },
                );
            ...
            0b0000_1101 => {
                ...
                if let Some(val) = val {
                    let enum_type = format!(
                        "app.WeaponDef.{}Id",
                        util::to_pascal_case(val.as_str()).replace("_", "")
                    );
                    self.add_enum_edit_ex(
                        type_info,
                        "FreeVal1",
                    ...
                    let artian_perf_type = self.get_enum_str_from_field(
                        "FreeVal2",
                        "app.ArtianDef.PERFORMANCE_TYPE_Fixed",
                        ctx,
                    );
                    self.add_enum_edit_ex(
                        type_info,
                        "FreeVal2",
                        "FreeVal2(PerformanceType)",
                        "app.ArtianDef.PERFORMANCE_TYPE_Fixed",
                        ui,
                        ctx,
                        &mut left,
                    ...
                    if Some("INVALID".to_string()) != artian_perf_type {
                        ...
                        if let Some(bonus_by_creating_mut) =
                            self.get_mut::<&mut u32>("BonusByCreating")
                        {
                            ...
                            let mut artian_skill_fixed =
                                ((skill_digit_1 * 100) + (skill_digit_2 * 10) + skill_digit_3)
                                    as i32;
                            ...
                            ui.horizontal(|ui| {
                                ui.label("Skills");
                                let child_id =
                                    ui.make_persistent_id((ctx.id, "Skills", hash)).value();
                                let mut child_ctx = Class::udpate_ctx(type_info, child_id, ctx);
                                edit_i32_enum_from_type(
                                    &mut artian_skill_fixed,
                                    "app.ArtianDef.ArtianSkillType_Fixed",
                                    ui,
                                    &mut child_ctx,
                                );
                            });
                        ...
                        if let Some(bonus_by_grinding_mut) =
                            self.get_mut::<&mut u64>("BonusByGrinding")
                        {
                            let bonus_by_grinding = *bonus_by_grinding_mut;
                            let mut bonus_1 =
                                ((bonus_by_grinding / 1_000_000_000_000) % 1000) as i32;
                            ...
                            ui.indent((hash, ctx.id), |ui| {
                                let child_id =
                                    ui.make_persistent_id((hash, ctx.id, "Bonus1")).value();
                                let mut child_ctx = Class::udpate_ctx(type_info, child_id, ctx);
                                edit_i32_enum_from_type(
                                    &mut bonus_1,
                                    "app.ArtianDef.BONUS_ID_Fixed",
                                    ui,
                                    &mut child_ctx,
                                );
                                let child_id =
                                    ui.make_persistent_id((hash, ctx.id, "Bonus2")).value();
                                let mut child_ctx = Class::udpate_ctx(type_info, child_id, ctx);
                                edit_i32_enum_from_type(
                                    &mut bonus_2,
                                    "app.ArtianDef.BONUS_ID_Fixed",
                                ...
                                let child_id =
                                    ui.make_persistent_id((hash, ctx.id, "Bonus3")).value();
                                let mut child_ctx = Class::udpate_ctx(type_info, child_id, ctx);
                                edit_i32_enum_from_type(
                                    &mut bonus_3,
                                    "app.ArtianDef.BONUS_ID_Fixed",
                                ...
                                let child_id =
                                    ui.make_persistent_id((hash, ctx.id, "Bonus4")).value();
                                let mut child_ctx = Class::udpate_ctx(type_info, child_id, ctx);
                                edit_i32_enum_from_type(
                                    &mut bonus_4,
                                    "app.ArtianDef.BONUS_ID_Fixed",
                                ...
                                let child_id =
                                    ui.make_persistent_id((hash, ctx.id, "Bonus5")).value();
                                let mut child_ctx = Class::udpate_ctx(type_info, child_id, ctx);
                                edit_i32_enum_from_type(
                                    &mut bonus_5,
                                    "app.ArtianDef.BONUS_ID_Fixed",
                    ...
                    if *free_flag_1 == 1 {
                        ...
                        let mut edit_skill = |ui: &mut Ui, idx: usize, skill: &mut i16| {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                let child_id =
                                    ui.make_persistent_id(("AmuletEdit", ctx.id, idx)).value();
                                let mut child_ctx = Class::udpate_ctx(type_info, child_id, ctx);
                                edit_i32_enum_from_type(
                                    &mut id,
                                    "app.HunterDef.Skill",
                            ...
                            if let Some(slots) = slots {
                                ...
                                ui.horizontal(|ui| {
                                    ComboBox::new((ctx.id, "AmuletSlots"), "")
                                        .selected_text(if slot_4 == 1 { "Weapon" } else { "Armor" })
                                        .show_ui(ui, |ui| {
    ...
    pub fn get_preview_from_field(
        &self,
        field_name: &str,
        ctx: &mut EditContext,
    ) -> Option<String> {
        self.get_field(field_name)
            .and_then(|x| x.value.get_preview(ctx))
    }
    ...
    pub fn get_preview(&self, ctx: &mut EditContext) -> Option<String> {
        ...
        type_info.and_then(|type_info| {
            match type_info.name.as_str() {
                    ...
                    if let Some(cat_gen_val) = self.get::<u8>("Category_Gender") {
                        ...
                        match cat_gen_val {
                            ...
                            0b0000_0000 | 0b0000_0001 => {
                                ...
                                let preview_series = self.get_enum_str_from_field(
                                    "FreeVal0",
                                    "app.ArmorDef.SERIES",
                                    ctx,
                                );
                                let preview_type = self.get_enum_str_from_field(
                                    "FreeVal1",
                                    "app.ArmorDef.ARMOR_PARTS",
                                    ctx,
                                );
                            ...
                            0b0000_1101 => {
                                ...
                                let preview = preview_type.and_then(|ty| {
                                    let enum_type = format!(
                                        "app.WeaponDef.{}Id",
                                        util::to_pascal_case(ty.as_str()).replace("_", "")
                                    );
                                    let preview_id =
                                        self.get_enum_str_from_field("FreeVal1", &enum_type, ctx);
                                    if let Some(id) = preview_id {
                                        Some(
                                            ctx.type_map
                                                .get_enum_text(&id, &enum_type, ctx.language)
                                                .map(|x| format!("{id}:{x}"))
                            ...
                            // idfk 21
                            0b0001_0101 => {
                                let preview_id = self.get_enum_str_from_field(
                                    "FreeVal0",
                                    "app.ArmorDef.AmuletType",
                                    ctx,
                                );
                ...
                _ => {
                    let r = self.to_string(ctx.language, ctx.type_map, ctx.remaps, ctx.assets);
                    return r;
                },
            }
        })
